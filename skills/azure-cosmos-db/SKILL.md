...
| Decision Making | L155-L209 | Guidance on choosing Cosmos DB and Cosmos DB for PostgreSQL/MongoDB options, including scaling, throughput, consistency, networking, pricing, and migration/upgrade paths from related services. |
| Architecture & Design Patterns | L210-L241 | Patterns and architectures for AI agents, vector search, multitenancy, microservices, high availability, change feed, analytics, and migrations using Azure Cosmos DB and Cosmos DB for PostgreSQL |
| Limits & Quotas | L242-L283 | Limits, quotas, and behaviors for Cosmos DB (and DocumentDB), including throughput, storage, backups/restore, free tier, serverless, partitioning, indexing, and emulator capabilities. |
| Security | L284-L364 | Securing Cosmos DB: identity/RBAC, keys and encryption, network isolation (firewalls, Private Link, VNet), auditing, data masking, TLS, and security best practices across all APIs. |
| Configuration | L365-L496 | Configuring Cosmos DB and related services: throughput, indexing, backup/restore, monitoring, search/vector, SDK tuning, Cassandra/Mongo/PostgreSQL setups, fleets, and emulator/infra provisioning. |
...
| Resolve issues with Azure Cosmos DB partial document updates | https://learn.microsoft.com/en-us/azure/cosmos-db/partial-document-update-faq |
